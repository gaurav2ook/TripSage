...
export class CreateNewPackageComponent {
 // Trip basic details
 ...
 addedPackages: any[] = []; // This is where packages are stored

 // Method to remove a package
 removePackage(packageToRemove: any): void {
   const index = this.addedPackages.indexOf(packageToRemove);
   if (index !== -1) {
     this.addedPackages.splice(index, 1); // Remove the package from the array
   }
 }
 ...
 createPackage(): void {
   ...
   console.log('Package created', this.addedPackages);
 }
